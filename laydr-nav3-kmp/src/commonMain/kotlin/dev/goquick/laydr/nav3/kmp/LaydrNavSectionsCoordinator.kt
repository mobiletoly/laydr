...
/**
 * Remembers internal Nav3 stack coordination for declared sections.
 */
@Composable
...
internal class LaydrNavSectionsCoordinator<Data : Any> internal constructor(
    ...
    /**
     * Controller that owns the currently selected section stack.
     */
    ...
    /**
     * Returns the single-stack controller for [section].
     */
    ...
    fun selectPath(path: String): LaydrNavPathResult =
        runtimeEngine.selectPath(path).toKmp()
    ...
    fun pushPath(path: String): LaydrNavPathResult =
        runtimeEngine.pushPath(path).toKmp()

    fun pushExternalTarget(input: String): LaydrNavExternalTargetResult =
        runtimeEngine.pushExternalTarget(input).toKmp()
    ...
    fun replacePath(path: String): LaydrNavPathResult =
        runtimeEngine.replacePath(path).toKmp()

    fun replaceExternalTarget(input: String): LaydrNavExternalTargetResult =
        runtimeEngine.replaceExternalTarget(input).toKmp()
