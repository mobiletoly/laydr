...
/**
 * Complete sectioned Nav3 runtime object returned by Laydr section helpers.
 *
 * The object exposes the selected section stack and entry provider for app
 * `NavDisplay` rendering, plus [navigator] for route-facing section
 * navigation. Labels, icons, visibility, chrome, auth, and presentation policy
 * remain app-owned.
 */
public class LaydrNavSections<Data : Any> internal constructor(
     ...
     * `null`.
     *
     * This is navigation state, not a visible-content callback from
     * Nav3 `NavDisplay`. App chrome that must stay synchronized with
     * rendered screen content should render route labels inside route entries
     * or manage its own transition state.
     */
    ...
    /**
     * True when app Back can restore a return-aware navigation point or pop
     * the selected section stack.
     */
    ...
    /**
     * Recognized list-detail route facts for the selected stack, or `null`.
     */
    ...
    /**
     * Returns whether app Back should be shown for the current stack and
     * app-owned list/detail pane policy.
     *
     * Pass `true` when a wide list/detail layout already shows both panes and
     * the selected detail should not display a stack-pop affordance.
     */
