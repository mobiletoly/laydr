...
/**
 * Result of resolving an external path into an AndroidX Nav3 stack operation.
 */
public sealed interface LaydrNavPathResult {
    ...
    /**
     * True when [path] resolved to a renderable Laydr AndroidX Nav3 key.
     */
    public val accepted: Boolean
}

/**
 * Accepted path result with the renderable AndroidX Nav3 [key].
 */
...
/**
 * Result of resolving a URL-like external target into an AndroidX Nav3 stack
 * operation.
 */
public sealed interface LaydrNavExternalTargetResult {
    ...
    /**
     * True when [input] resolved to a renderable Laydr AndroidX Nav3 key.
     */
    public val accepted: Boolean
}

/**
 * Accepted external target result with the renderable AndroidX Nav3 [key].
 */
...
/**
 * Rejected external target lookup.
 */
...
/**
 * Reason an external path could not be turned into a Laydr AndroidX Nav3
 * entry.
 */
public typealias LaydrNavPathRejectionReason = dev.goquick.laydr.nav.runtime.LaydrNavPathRejectionReason

/**
 * Reason an external target could not be turned into a Laydr AndroidX Nav3
 * entry.
 */
...
/**
 * Converts [path] to a Laydr AndroidX Nav3 key, or returns `null`.
 */
...
/**
 * Resolves [path] to an accepted or rejected Laydr path result.
 */
public fun laydrNavPathResult(
    path: String,
    routeMap: LaydrRouteMap,
): LaydrNavPathResult =
    dev.goquick.laydr.nav.runtime.laydrNavPathResult(path = path, routeMap = routeMap)
        .toAndroidx()

/**
 * Resolves a URL-like [input] to an accepted or rejected Laydr target result.
 */
public fun laydrNavExternalTargetResult(
    input: String,
    routeMap: LaydrRouteMap,
): LaydrNavExternalTargetResult =
    dev.goquick.laydr.nav.runtime.laydrNavExternalTargetResult(input = input, routeMap = routeMap)
        .toAndroidx()

/**
 * Builds the concrete Laydr path represented by this AndroidX Nav3 key.
 */
public fun LaydrNavKey.path(routeMap: LaydrRouteMap): String? =
    entryKey.path(routeMap)

internal fun dev.goquick.laydr.nav.runtime.LaydrNavPathResult.toAndroidx(): LaydrNavPathResult =
    when (this) {
...
internal fun dev.goquick.laydr.nav.runtime.LaydrNavExternalTargetResult.toAndroidx(): LaydrNavExternalTargetResult =
    when (this) {
